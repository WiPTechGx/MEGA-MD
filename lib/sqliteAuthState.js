...
async function useSQLiteAuthState() {
    ...
    // Check if session/creds.json has newer/different creds (e.g. session was refreshed or switched)
    const credsPath = path.join(__dirname, '..', 'session', 'creds.json');
    if (fs.existsSync(credsPath)) {
        try {
            const fileContent = fs.readFileSync(credsPath, 'utf8');
            const fileCreds = JSON.parse(fileContent);
            const fileMeId = fileCreds?.me?.id;
            const sqliteMeId = creds?.me?.id;

            if (fileMeId && (!sqliteMeId || fileMeId !== sqliteMeId)) {
                console.log(`[AUTH] Session mismatch detected! File: ${fileMeId} vs SQLite: ${sqliteMeId || 'none'}. Re-importing session file...`);
                creds = importCredsFromFile();
            } else if (fileCreds?.registered === true && creds?.registered === false) {
                console.log(`[AUTH] Session registration state mismatch! Re-importing session file...`);
                creds = importCredsFromFile();
            }
        } catch (e) {
            console.error('[AUTH] Error checking session file sync:', e.message);
        }
    }
